-- Remove the old trigger that directly inserts alerts
DROP TRIGGER IF EXISTS check_subcontractor_amounts_trigger ON subcontracts;

-- Update the function to not insert alerts directly, but use a notification
...
BEGIN
  ...
  IF project_subcontractor_total > 5000000 THEN
    ...
    IF existing_alert_id IS NULL THEN
      -- Create new alert and notify the application to send emails
      INSERT INTO alerts (
      ...
      -- Notify the application about the new alert
...
CREATE TRIGGER check_subcontractor_amounts_trigger
  AFTER INSERT OR UPDATE ON subcontracts
  FOR EACH ROW
